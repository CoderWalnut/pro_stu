...
xnet_packet_t* xnet_alloc_for_recv(uint16_t data_size) {
    rx_packet.data = tx_packet.payload;
...
static remove_header(xnet_packet_t* packet, uint16_t header_size) {
    packet->data += header_size;
    packet->size -= header_size;
}
...
static xnet_err_t ethernet_out_to(xnet_protocol_t protocol, const uint8_t* mac_addr, xnet_packet_t* packet) {
    ...
    memcpy(ether_hdr->dst_mac_addr, mac_addr, XNET_MAC_ADDR_SIZE);
    memcpy(ether_hdr->src_mac_addr, netif_mac, XNET_MAC_ADDR_SIZE);
...
static void ethernet_in(xnet_packet_t* packet) {
    ...
    xether_hdr_t* ether_hdr = packet->data;
    switch (ether_hdr->protocol) {
